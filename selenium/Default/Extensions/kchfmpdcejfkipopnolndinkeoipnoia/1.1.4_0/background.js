...
chrome.extension.onRequest.addListener(function(request, sender, sendResponse) {
    if (request.action == "blacklist") getSpoofValues(sender.tab.url, function(values) {
        sendResponse(values);
    });
    else if (request.action == "js_blacklist") getDoNotSpoofJSObjects(function(dont_spoof_js) {
        if (dont_spoof_js) sendResponse({});
        else getSpoofValues(sender.tab.url, function(values) {
            sendResponse(values);
        });
    });
    ...
    } else if (request.action == "options") _getUserAgentList(function(user_agent_list) {
        sendResponse({
        ...
        updateListeners();
        sendResponse({});
    } else if (request.action == "clear_presets") clearPresets();
    else if (request.action == "badge") updateBadge(sender.tab);
    else {
...
function replaceHeader(user_agent, requestHeaders) {
    if (!user_agent || !user_agent.ua_string) return requestHeaders;
    var newHeaders = [];
    for (var i = 0; i < requestHeaders.length; i++)
        if (requestHeaders[i].name != "User-Agent") newHeaders.push(requestHeaders[i]);
        else {
            var new_value = requestHeaders[i].value;
            if (user_agent.ua_string != "") new_value = (user_agent.append_to_default_ua ? new_value + " " + user_agent.ua_string : user_agent.ua_string);
            newHeaders.push({
                "name": "User-Agent",
                "value": new_value
            });
...
function updateListeners() {
    if (!listener) listener = function(details) {
        ...
        if (details && details.url && details.requestHeaders && details.requestHeaders.length > 0) header_map = {
