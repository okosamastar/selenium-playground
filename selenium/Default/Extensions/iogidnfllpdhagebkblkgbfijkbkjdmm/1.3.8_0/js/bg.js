...
WebExtensions.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (cmd === CMD_INIT) {
    ...
    const pageCounter = params.counter || 0;
    const version = manifest?.version || "0.0.0";
    if (option.counter < pageCounter) {
      option.counter = pageCounter;
      saveOption(option);
    }
    const counter = option.counter;
    if (tab.title) {
      return sendResponse({id, parentTabId, url:tab.url, title:tab.title, counter, version});
    } else {
      if (("" + tab.id).startsWith("virtual_")) {
        return sendResponse({id, parentTabId, url:tab.url, title:"video title - none", counter, version});
      } else {
