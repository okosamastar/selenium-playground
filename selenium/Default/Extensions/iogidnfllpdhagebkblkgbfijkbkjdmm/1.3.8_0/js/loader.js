...
class MP4 {
  ...
  async clone() {
    const data = await this.publishAsync();
    return new MP4(data);
  }
  static get Box() {
    return Box;
...
const _buildMP4 = async(data, flags, video) => {
  ...
  for (const d of data) {
    let mp4;
    if (d.init && d.init.data) {
      mp4 = d.init.data.length < 20000 ? new MP4(d.init.data.slice()) : d.init;
    } else {
      mp4 = await d.init.clone();
    }
    mp4s.push(mp4);
    ...
    for (let n = 0; n < len; n++) {
      for (let i = 0; i < 2; i++) {
        ...
        for (const trun of traf.truns) {
          trun.samples = trun.samples || (new Array(trun.sampleCount || 0)).fill({});
    ...
    if (fragment) {
      ...
      if (error) {
        if (loadPtr < fragments.length - 1) {
          ...
          showErrorMessage("Failed to fetch TS, abort, status : " + (error.status || "Possibly HLS-encryption"));
          break;
        }
      }
      try {
...
const onFragParsingData_AV = (video, frag, data) => {
  ...
  const s = new MP4(new Uint8Array(data1));
  const tracks = divideByTrack(s);
  if (tracks.length === 1) {
    const src = tracks[0];
    for (let n = 0; n < src.root.moofs.length; n++) {
      const moof = src.root.moofs[n];
      const dest = video.getTrackByNumber(moof.traf.tfhd.trackID);
      const pos = dest.moofs.length;
      dest.moofs[pos] = moof;
      dest.mdats[pos] = new Blob([src.root.mdats[n].data]);
      dest.start[pos] = frag.cc * 1000000000 + pos;
    }
    for (let n = 0; n < video.tracks.length; n++) {
      video.tracks[n].duration += frag.duration;
    }
  } else {
    const pos = frag._n;
...
const onFragParsingData = (video, frag, data) => {
  ...
  if (data1 && !data2) {
    ...
    for (let n = 0; n < mp4.root.moofs.length; n++) {
      const p = dest.moofs.length;
      const moof = mp4.root.moofs[n];
      const moofS = moof.offset;
      const moofE = moofS + moof.wholeSize;
      if (option.priority === PRIORITY.SPEED) {
        boxAsLightWeight(moof);
        dest.moofs[p] = moof;
      } else {
        dest.moofs[p] = new Blob([data1.slice(moofS, moofE)]);
      }
      dest.mdats[p] = new Blob([mp4.root.mdats[n].data]);
      dest.start[p] = frag.cc * 1000000000 + frag.sn;
      totalsize += moofE - moofS + dest.mdats[p].size;
    }
  } else {
...
const init = async() => {
  const tabInfo = await sendMessage({cmd:CMD_INIT, params:{counter:option.counter}});
  if (tabInfo) {
    if (_LOG_) {
    ...
    if (!tabInfo.error) {
      TAB = new Tab(tabInfo);
      if (tabInfo.counter > option.counter) {
        option.counter = tabInfo.counter;
        option.version = tabInfo.version;
        saveOption(option);
      }
    }
  }
